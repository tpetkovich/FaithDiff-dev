            ...
            if  self.use_rgb == False:
                ...
                if is_torch_version(">=", "1.11.0"):
                    for idx, down_block in enumerate(self.down_blocks):
                        ...
                        pixel_to_rgb = torch.utils.checkpoint.checkpoint(
                            create_custom_forward(self.to_rgbs[idx]), sample, use_reentrant=False
                        )
                    # middle
                        pixel_list.append(pixel_to_rgb)
                    sample = torch.utils.checkpoint.checkpoint(
                        create_custom_forward(self.mid_block), sample, use_reentrant=False
                    )
                else:
                    for idx, down_block in enumerate(self.down_blocks):
                        sample = torch.utils.checkpoint.checkpoint(create_custom_forward(down_block), sample)
                        pixel_to_rgb = torch.utils.checkpoint.checkpoint(
                            create_custom_forward(self.to_rgbs[idx]), sample
                        )
                        pixel_list.append(pixel_to_rgb) 
                    # middle
                    sample = torch.utils.checkpoint.checkpoint(create_custom_forward(self.mid_block), sample)

                return sample, pixel_list
...
class UNet2DConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin, PeftAdapterMixin):
    ...
    def init_ControlNetConditioningEmbedding(self, channel=512):
        self.condition_embedding = Gate_ControlNetConditioningEmbedding(320, channel)
    ...
    def get_aug_embed(
        ...
        elif self.config.addition_embed_type == "only_time":
            # SDXL - style
            if "text_embeds" not in added_cond_kwargs:
                raise ValueError(
                    f"{self.__class__} has the config param `addition_embed_type` set to 'text_time' which requires the keyword argument `text_embeds` to be passed in `added_cond_kwargs`"
    ...
    def forward(
        ...
        if input_embedding != None:
            if use_condition_embedding == True:
                # print(input_embedding.size())
                input_embedding = self.condition_embedding(input_embedding)
            # mean_noise = torch.mean(sample, dim=(2, 3)).unsqueeze(1)
            # mean_input_embedding = torch.mean(input_embedding, dim=(2, 3)).unsqueeze(1)
            ...
            # print(f"concat_feat.size() {concat_feat.size()}")
            feat_alpha = self.spatial_ch_projs(concat_feat).transpose(1,2).view(batch_size, channel, height, width)
            # print(f"feat_alpha.size() {feat_alpha.size()}")
            # feat_alpha = feat_alpha.unsqueeze(-1).unsqueeze(-1)
            if add_sample:
                sample = sample + feat_alpha #+ input_embedding #
            else:
                sample = feat_alpha
